---
title: "Upgrade Guide: Laravel QR Code v1.x to v2.x Migration"
sidebarTitle: "Upgrade Guide"
description: "Complete upgrade instructions for moving from linkxtr/laravel-qrcode v1.x to v2.x, including breaking changes, required code updates, and rollback steps."
---

Version 2.x is a significant release that upgrades the underlying `bacon/bacon-qr-code` library from v2 to v3, hardens the API with strict types, and drops legacy compatibility layers. Most applications need only a few targeted changes to upgrade cleanly.

<Note>
  **Laravel 10 users:** v2.x requires Laravel 11+ and PHP 8.2+. If you are on Laravel 10 or PHP 8.1, stay on v1.x — it receives maintenance fixes and is the supported path for those environments.
</Note>
...
<Accordion title="5. Strict data validation — InvalidArgumentException on bad input">
  In v1.x, passing malformed data to type methods (such as a bad phone number or an incomplete WiFi config) could silently generate a corrupted QR code. In v2.x, all data types validate their inputs strictly and throw `InvalidArgumentException` on failure.
...
<Accordion title="6. Config defaults changed in v2.4.x">
  ...
  | Key | v1.x / pre-2.4 default | v2.4.x+ default |
  |---|---|---|
  | `format` | `png` | `svg` |
  | `size` | `200` | `400` |
...
<Steps>
  <Step title="Update the package constraint">
    ...
    Composer will pull in the latest v2.x release along with `bacon/bacon-qr-code` v3.
  </Step>
...
<Warning>
  If you published `config/qrcode.php` during the v2.x migration and changed values, review the config after rolling back — the v1.x config schema may differ from v2.x.
</Warning>
