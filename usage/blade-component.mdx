...
<Tip>
  The Blade component is the recommended choice for templates. It handles HTML escaping, accessibility markup, and binary-to-base64 conversion automatically. For programmatic generation in controllers or jobs, use the [QrCode facade](/usage/facade) instead.
</Tip>
...
<Warning>
  EPS format is not supported in the Blade component. Using `format="eps"` throws an `InvalidConfigurationException`. For EPS output, use the [QrCode facade](/usage/facade) directly.
</Warning>
