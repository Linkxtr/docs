...
<Note>
  Although the constructor accepts integers and floats for convenience (e.g. when pulling from a numeric database column), always prefer a string prefixed with `+` and the country code to ensure the dialler opens correctly on every device.
</Note>
