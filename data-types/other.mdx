...
<ParamField body="encryption" type="string">
  The security protocol: `WEP`, `WPA`, `WPA2`, `WPA3`, or `NOPASS`. `WPA2` and
...
<ParamField body="password" type="string">
  The network password. Omit for open (`NOPASS`) networks. Passing a password
  alongside `NOPASS` throws an `InvalidWiFiArgumentException`.
</ParamField>
...
<Warning>
  **WEP is deprecated and insecure.** It is supported only for compatibility
  with legacy hardware. Use `WPA` or `WPA2` for all new deployments.
</Warning>
...
<ParamField body="latitude" type="float" required>
  Decimal latitude between `-90` and `90`. Values outside this range throw an
  `InvalidGeoArgumentException`.
</ParamField>

<ParamField body="longitude" type="float" required>
  Decimal longitude between `-180` and `180`. Values outside this range throw an
  `InvalidGeoArgumentException`.
</ParamField>
...
<ParamField body="start" type="Carbon|DateTimeInterface|string|int" required>
  The event start date and time. Accepts a `Carbon` instance, any
...
<ParamField body="end" type="Carbon|DateTimeInterface|string|int" required>
  The event end date and time. Must be strictly after `start`; otherwise an
  `InvalidCalendarEventArgumentException` is thrown.
</ParamField>
...
<Note>
  Per RFC 5545, the `DTSTAMP` field is set to the **current UTC timestamp**
...
<Tip>
  All timestamps are converted to UTC before being written into the payload,
  regardless of the timezone on the `Carbon` instance you pass in. Pass
  timezone-aware `Carbon` objects (e.g. `Carbon::create(...,
  'America/Chicago')`) to ensure the event appears at the correct local time on
